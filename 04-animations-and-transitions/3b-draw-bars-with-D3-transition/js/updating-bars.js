async function updatingBars() {
  ...
  const drawHistogram = metric => {
    ...
    //   .duration(600)
    //   /*
    //     NOTE: Some transitions will cause our bars to bounce below the x axis when they
    // animate; which will appear as errors in the JavaScript console along the lines of:

    //       Error: <rect> attribute height: A negative value is not valid.

    //     This is expected behavior.

    //     See https://github.com/d3/d3-ease/blob/v1.0.5/README.md for visualizations
    // and D3 easing functions
    //   */
    ...
    // IMPROVEMENT: Our bars are moving to their new positions while bars are exiting
    // and displaying intermediate states...
    // Let's fix that by delaying the update transition until the exit transition has finished
    const exitTransition = d3.transition().duration(600)
    const updateTransition = exitTransition.transition().duration(600)
    // Alternatively, we could use the .delay() method -
    ...
    newBinGroups
      .append('rect')
      // IMPROVEMENT: Start in the right horizontal location and be 0 pixels tall
      // to prevent bars from flying in from the left
      .attr('height', 0)
      .attr('x', d => xScale(d.x0) + barPadding)
      .attr('y', dimensions.boundedHeight)
      .attr('width', d => d3.max([0, xScale(d.x1) - xScale(d.x0) - barPadding]))
      // IMPROVEMENT: We need fill to be an inline style so that we can override it in our CSS
      // file; otherwise this would just be an SVG attribute
      .style('fill', 'yellowgreen')

    newBinGroups.append('text')
      // IMPROVEMENT: Set our labels' initial position to prevent them from flying
      // in from the left
    ...
    const xAxis = bounds.select('.x-axis')
      .transition(updateTransition) // IMPROVEMENT: Use our custom transition from above
      //We can now see our tick marks move to fit the new domain before
      // the new tick marks are drawn
      .call(xAxisGenerator)
