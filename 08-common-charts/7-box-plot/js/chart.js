async function drawBars() {
  ...
  //     .entries(dataset)
  let dataByMonth = Array.from(
    d3.group(dataset, d => formatAsMonth(dateParser(d.date))), ([key, value]) => ({ key, value })
  );
  ...
  const dataByMonthWithStats = dataByMonth.map(month => {
    const monthYValues = month.value.map(yAccessor).sort((a, b) => a - b)
    ...
    const outliers = month.value.filter(d => yAccessor(d) < rangeMin || yAccessor(d) > rangeMax)
