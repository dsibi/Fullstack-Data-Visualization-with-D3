...
NUM_OF_LEVEL = 10,
    size = Math.min(window.innerWidth, window.innerHeight, 358),
    offset = Math.PI,
...
const createSVG = (dataset) => {
    ...
    for (let index = 0; index < 20; index++) {
        ...
        const rank = program_type == 'fs' ?
            d3.select("#j" + element + " .rank")
                // .text('КП ' + dataset[index].value[0].tss_sp + ' + ПП ' +
                    .text(dataset[index].value[0].final_rank + ' место: КП ' + dataset[index].value[0].tss_sp + ' + ПП ' +
        ...
        const skater_name = d3.select("#j" + element + " .skater_name")
            .text(dataset[index].value[0].name_rus);
        const score = program_type == 'fs' ? d3.select("#j" + element + " .score")
            .text("Техн " + dataset[index].value[0].total_element_score + " + Комп " + dataset[index].value[0].total_component_score
                + " - Штрафы " + dataset[index].value[0].total_deductions + " = " + dataset[index].value[0].total_segment_score) :
